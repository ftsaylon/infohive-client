...
class AddDistributor extends Component{
    ...
    handleSubmit = event => {
        ...
        if(this.formValidation()){
            axios.post(`/distributor/`, contact )
            .then(res => {
                // console.log(res);
                // console.log(res.data);
                this.props.updateList("distributor", 'added');
                this.clearFields();
                this.closeModal();
            })
            .catch(e => {
    ...
    render(){
        ...
        return(
            <div>
                <Modal size="small" closeIcon onClose={this.closeModal} open={showModal} trigger={<Button onClick={() => this.setState({ showModal: true })}>Add Distributor</Button>}>
                    <Modal.Header>Add Distributor</Modal.Header>
                    <Modal.Content>
                    <Form onSubmit={this.handleSubmit}>
                        <Form.Field required onChange={this.handleChange} error={nameError}>
                            <label>Distributor Name</label>
                            <input required value={name} name="name" placeholder='ex. Mister Honey Bee' maxLength={50}/>
                        </Form.Field>
                        ...
                        <Form.Group>
                            <Form.Field width={4} required onChange={this.handleChange} error={phoneError}>
                                ...
                                <input type='number' required value={phone} name="phone" placeholder='ex.  09123456789'maxLength={20}/>
                            </Form.Field>
                            <Form.Field width={12} onChange={this.handleChange} error={webUrlError}>
                                <label>Web URL</label>
                                <input value={webUrl} name="webUrl" placeholder="ex. https://google.com" maxLength={30}/>
                            </Form.Field>
                        </Form.Group>
